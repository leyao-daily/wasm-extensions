...
bool ExampleRootContext::onConfigure(size_t configuration_size) {
  ...
  if (it != j.end()) {
    ...
    rules_service_ = rules_service.first.value();
  } else {
    LOG_WARN(
        absl::StrCat("rules service must be provided in plugin "
  ...
  if (it != j.end()) {
    ...
    proxy_set_tick_period_milliseconds(std::stoi(duration_time.first.value()));
  } else {
    LOG_WARN(
        absl::StrCat("duration time must be provided in plugin "
                     "configuration JSON string: ",
                     configuration_data->view()));
    return false;
  }
